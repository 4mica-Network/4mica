  ...
  return (
    <section id="use-cases" className="section-gloss py-24">
      <div className="mx-auto w-full max-w-300">
        ...
        <div className="grid w-full grid-cols-1 overflow-hidden rounded-md border border-overlay/10 bg-surface-deep/25 md:grid-cols-2">
          {USE_CASES.map((uc, index) => {
            ...
            return (
              <div
                key={uc.title}
                className="group relative flex min-w-0 flex-col gap-4 border-overlay/10 border-b p-6 transition-colors duration-500 hover:bg-overlay/[0.018] sm:p-7 md:nth-last-[-n+2]:border-b-0 md:odd:border-r"
              >
                <div
                ...
                <div className="mt-auto flex flex-wrap gap-2 pt-2">
                  {uc.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-md border border-overlay/10 bg-overlay/2.5 px-2 py-0.5 font-medium text-ink-muted text-md transition-colors duration-500 group-hover:border-overlay/15 group-hover:text-ink-body"
                    >
                      {tag}
                    </span>
                  ))}
