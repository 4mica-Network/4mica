        ...
        <div className="grid w-full grid-cols-1 overflow-hidden rounded-md border border-white/10 bg-black/25 md:grid-cols-2">
          {USE_CASES.map((uc, index) => {
            ...
            return (
              <div
                key={uc.title}
                className="group relative flex min-w-0 flex-col gap-4 border-white/10 border-b p-6 transition-colors duration-500 hover:bg-white/[0.018] sm:p-7 md:odd:border-r md:[&:nth-last-child(-n+2)]:border-b-0"
              >
                <div
                ...
                <div className="mt-auto flex flex-wrap gap-2 pt-2">
                  {uc.tags.map((tag) => (
                    <span
                      key={tag}
                      className="rounded-md border border-white/10 bg-white/[0.025] px-2 py-0.5 font-medium text-ink-muted text-md transition-colors duration-500 group-hover:border-white/15 group-hover:text-ink-body"
                    >
                      {tag}
                    </span>
                  ))}
