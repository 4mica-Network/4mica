...
For server-side protection, use `paymentMiddlewareFromConfig` from `@4mica/x402/server/express`. The middleware returns HTTP 402 responses, verifies retried payment headers, settles the guarantee, and serves the protected route after payment succeeds.
...
2. The wrapper validates the payment requirements and creates a unique request ID.
...
<Columns cols={2}>
  ...
  <Card title="Hono" icon="zap" href="/quick-start/hono">
    Add paid endpoints to a Hono service while keeping the 4Mica guarantee flow explicit.
  </Card>
