...
```bash npm
npm install @4mica/x402@alpha @x402/fetch @x402/axios viem
```

```bash pnpm
pnpm install @4mica/x402@alpha @x402/fetch @x402/axios viem
```

```bash yarn
yarn add @4mica/x402@alpha @x402/fetch @x402/axios viem
```

```bash bun
bun add @4mica/x402@alpha @x402/fetch @x402/axios viem
```
...
const baseScheme = await FourMicaEvmScheme.create(
  privateKeyToAccount("0xBasePrivateKey"),
);

const fetchWithPayment = wrapFetchWithPaymentFromConfig(fetch, {
  schemes: [
    { network: "eip155:84532", client: baseSepoliaScheme },
    { network: "eip155:8453", client: baseScheme },
  ],
});
```
