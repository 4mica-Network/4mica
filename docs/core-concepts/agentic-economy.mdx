...
The HTTP specification reserved status code 402, "Payment Required," in 1997. No one implemented it. The internet carries information. Commerce grew on top of card networks and banking infrastructure that predates the web.

Agents expose that gap at scale. Filling it requires more than a single new protocol. A complete agentic commerce stack addresses six distinct problems, each requiring its own layer:
...
These layers are not redundant. They solve different problems. Protocols at each layer are actively competing. Some will consolidate. Others will disappear. But the structure reflects real, distinct requirements that emerge whenever software replaces humans as the transacting party.
