...
The BLS certificate is the payment authorization.
...
In x402 flows, the SDK often gets this value from a tab helper as `nextReqId`.
That helper may be backed by Core in older tab-based deployments.

The current cycle-native Core issuance path, `/core/guarantees`, does not generate `req_id`.
...
| `req_id`            | `uint256` | Payer / SDK or tab helper   | Request counter.                   |
