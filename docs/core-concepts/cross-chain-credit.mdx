...
- CAIP-2 identifies the network unambiguously;
...
Think of cross-chain credit as a shared language spoken by separate economic
systems:

> The payment pattern travels across networks. The collateral does not travel
> unless an explicit transfer mechanism moves it.

That model keeps the important boundaries visible:

- network selection happens before signing;
- collateral and capacity belong to one deployment;
- assets are identified by network and contract;
- guarantees use network-specific signing and Core configuration;
- clearing and settlement remain local to the deployment;
- bridging and cross-network netting are separate systems;
- multi-network convenience never removes the need for per-network operations.
