...
test token, `0xba50Cd2A20f6DA35D788639E581bca8d0B5d4D5f`, not Circle's, and anyone can mint it. Minting
is an on-chain call, so the wallet sending it needs a small amount of Base Sepolia ETH for gas — get
some from a [Base Sepolia faucet](https://docs.base.org/base-chain/network-information/network-faucets)
first. This is the only step that needs ETH; the deposit into 4Mica right after stays gasless.
...
gasless through the facilitator, so the wallet needs no further ETH for it. `GET /core/tokens` on the
...
```bash
pnpm install @4mica/x402@alpha @x402/fetch viem
```
...
const scheme = await FourMicaEvmScheme.create(account, {
  // The 2.0 protocol runs at staging.api.4mica.io while Base Sepolia's
  // permanent host is upgraded; the package default still points at the old one.
  coreUrls: { "eip155:84532": "https://staging.api.4mica.io" },
});
...
// Staging hosts while Base Sepolia's permanent ones are upgraded; the
// package defaults still point at the old deployment.
const network = "eip155:84532";
const coreUrl = "https://staging.api.4mica.io";
const facilitatorUrl = "https://staging.facilitator.4mica.io";
  ...
  paymentMiddlewareFromConfig(
    {
      "GET /premium-content": {
        accepts: {
          ...
          network,
          payTo: "0xYourAddress",
          // Tells payers which core to sign against, so a buyer on the
          // package default still lands on the right deployment.
          extra: { rpcUrl: coreUrl },
        },
      },
    },
    new FourMicaFacilitatorClient({ url: facilitatorUrl }),
    [{ network, server: new FourMicaEvmScheme({ coreUrls: { [network]: coreUrl } }) }],
  ),
);
```
