...
```bash npm
npm install @4mica/sdk@alpha
```

```bash pnpm
pnpm install @4mica/sdk@alpha
```

```bash yarn
yarn add @4mica/sdk@alpha
```

```bash bun
bun add @4mica/sdk@alpha
```
...
The default network is Base Sepolia. Select another with `.network()` or the `4MICA_NETWORK` environment variable.
...
| `ethereum-sepolia` | `eip155:11155111` | `https://ethereum.sepolia.api.4mica.xyz/` (legacy, x402 v1 only) |

<Warning>
The `base-sepolia` entry above is the package's built-in default and still points at the previous
deployment. The 2.0 protocol runs at `https://staging.api.4mica.io/` while that host is upgraded, so
set `.rpcUrl("https://staging.api.4mica.io/")` (or `4MICA_RPC_URL`) and
`.facilitatorUrl("https://staging.facilitator.4mica.io")` explicitly until then. This note goes away
when the permanent hosts serve 2.0.
</Warning>
...
const cfg = new ConfigBuilder()
  .network("base-sepolia") // the default
