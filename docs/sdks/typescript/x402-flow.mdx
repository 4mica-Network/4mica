...
Before signing, `X402Flow` reads the next request id from `extra.reqId` (defaulting to `0x0` when absent). Both signing methods always use EIP-712 and error if the scheme is not a 4Mica scheme.
