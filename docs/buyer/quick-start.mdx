...
4Mica lets buyers give an AI agent money without giving it unlimited trust. You fund a wallet, define what the agent is allowed to pay for, and keep a record of every paid request so spending stays visible, bounded, and auditable.

## What buyers control

When an agent can spend, the important question is not just "can it pay?" It is "can it pay within the rules I set?"

As a buyer, you should control:
...
<Steps>
  ...
  <Step title="Audit the task">
    Record each payment with the seller, route, amount, reason, tab, request ID, and task result.
  </Step>
</Steps>
...
- whether the tab or settlement obligation is still open.
