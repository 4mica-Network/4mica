...
<RequestExample>
```bash cURL
curl -X POST "https://staging.api.4mica.io/auth/nonce" \
  -H "Content-Type: application/json" \
  -d '{"address":"0x1111111111111111111111111111111111111111"}'
```
```javascript title="Fetch"
const response = await fetch("https://staging.api.4mica.io/auth/nonce", {
  method: "POST",
...
const { data } = await axios.post(
  "https://staging.api.4mica.io/auth/nonce",
...
```python Python
import requests
url = "https://staging.api.4mica.io/auth/nonce"
...
func main() {
  ...
  response, _ := http.Post("https://staging.api.4mica.io/auth/nonce", "application/json", bytes.NewReader(body))
