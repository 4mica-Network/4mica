...
<RequestExample>
```bash cURL
curl "https://staging.facilitator.4mica.io/supported"
```
```javascript title="Fetch"
const response = await fetch("https://staging.facilitator.4mica.io/supported");
console.log(await response.json());
```
```javascript title="Axios"
import axios from "axios";
const { data } = await axios.get("https://staging.facilitator.4mica.io/supported");
console.log(data);
```
```python Python
import requests
print(requests.get("https://staging.facilitator.4mica.io/supported").json())
```
```go Go
package main
import ("fmt"; "io"; "net/http")
func main() {
  response, _ := http.Get("https://staging.facilitator.4mica.io/supported")
