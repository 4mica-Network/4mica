...
<RequestExample>
```bash cURL
curl -X POST "https://staging.facilitator.4mica.io/verify" \
...
const response = await fetch("https://staging.facilitator.4mica.io/verify", {
  method: "POST",
...
const body = { x402Version: 1, paymentPayload, paymentRequirements };
const { data } = await axios.post("https://staging.facilitator.4mica.io/verify", body);
console.log(data);
```
```python Python
import requests
...
print(requests.post("https://staging.facilitator.4mica.io/verify", json=body).json())
```
```go Go
package main
...
func main() {
  ...
  response, _ := http.Post("https://staging.facilitator.4mica.io/verify", "application/json", bytes.NewReader(body))
